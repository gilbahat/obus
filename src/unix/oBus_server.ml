            ...
            try%lwt
              ...
              if nonce <> server.srv_nonce then begin
                let%lwt () = Lwt_log.notice_f ~section
                  "client rejected because of invalid nonce" in
                Lwt.return `Drop
              end else
                Lwt.return `OK
            with
              | End_of_file ->
                  let%lwt () = Lwt_log.warning ~section
                    "cannot read nonce from socket" in
                  Lwt.return `Drop
              | Unix.Unix_error(err, _, _) ->
                  let%lwt () = Lwt_log.warning_f ~section
                    "cannot read nonce from socket: %s" (Unix.error_message err) in
                  Lwt.return `Drop
          end >>= function
            | `OK ->
            ...
            | `Drop ->
                let%lwt () =
                  ...
                  with Unix.Unix_error(err, _, _) ->
                    Lwt_log.error_f ~section "cannot shutdown socket: %s"
    ...
    | "unix" -> begin
        match OBus_address.arg "path" address with
          | Some path -> begin
              ...
              with Unix_error(err, _, _) ->
                Lwt_log.error_f ~section "cannot unlink '%s': %s"
      ...
      | 1 ->
          let user_id =
            ...
            with Unix.Unix_error(error, _, _) ->
              ignore (Lwt_log.info_f ~section "cannot read credential: %s"
                (Unix.error_message error));
              None
          in
          let%lwt uid, capabilities =
            OBus_auth.Server.authenticate
              ...
              ()
          in
          if uid = None && not server.srv_allow_anonymous then begin
            let%lwt () = Lwt_log.notice_f ~section
              "client from %s rejected because anonymous connections are not allowed"
              (string_of_address address) in
            Lazy.force shutdown
          end else begin
            try
              server.srv_callback server
                (OBus_transport_unix.socket_of_fd ~capabilities fd);
              Lwt.return ()
            with exn ->
              let%lwt () = Lwt_log.error ~section ~exn
                "server callback failed with" in
              Lazy.force shutdown
          end
      | _ ->
          assert false
      ...
      match exn with
        | OBus_auth.Auth_failure msg ->
            Lwt_log.notice_f ~section
              "authentication failure for client from %s: %s"
              (string_of_address address) msg
        | exn ->
            Lwt_log.error_f ~section ~exn
              "authentication for client from %s failed with"
  ...
  Lwt.pick [server.srv_abort_waiter; accept server listener] >>= function
    | Event_shutdown ->
        let%lwt () =
          ...
          with Unix_error(err, _, _) ->
            Lwt_log.error_f ~section "cannot close listening socket: %s"
              (Unix.error_message err)
        in
...
let make_socket domain typ address =
  ...
  with Unix_error(err, _, _) as exn ->
    let%lwt () = Lwt_log.error_f ~section
      "failed to create listening socket with %s: %s"
      (string_of_address address) (Unix.error_message err) in
    let%lwt () = Lwt_unix.close fd in
    Lwt.fail exn

let make_path path   = make_socket PF_UNIX SOCK_STREAM (ADDR_UNIX path)
let make_abstract p  = make_socket PF_UNIX SOCK_STREAM (ADDR_UNIX("\x00" ^ p))

let fd_addr_list_of_address address =
  match OBus_address.name address with
    | "unix" -> begin
        match (OBus_address.arg "path" address,
          ...
          | None, None, Some tmpd -> begin
              let path =
                Filename.concat tmpd
                  ("obus-" ^ OBus_util.hex_encode (OBus_util.random_string 10))
              in
              try%lwt
                let%lwt fd = make_abstract path in
                Lwt.return [(fd, OBus_address.make ~name:"unix"
                               ~args:[("abstract", path)])]
              with _ ->
                let%lwt fd = make_path path in
                Lwt.return [(fd, OBus_address.make ~name:"unix"
          ...
          | _ ->
              Lwt.fail (Invalid_argument
                "OBus_server.make: invalid unix address; \
                 supply exactly one of 'path', 'abstract', 'tmpdir'")
      end

    | ("tcp" | "nonce-tcp") as name -> begin
        let port  = Option.value (OBus_address.arg "port" address)  ~default:"0"
        and bind_ = match OBus_address.arg "bind" address with
          | Some b -> b
          | None   -> Option.value (OBus_address.arg "host" address) ~default:"*"
        in
        ...
        let opts = match OBus_address.arg "family" address with
          ...
          | Some f ->
              Printf.ksprintf invalid_arg
                "OBus_server.make: unknown address family '%s'" f
          | None -> opts
        in
        let ais = getaddrinfo bind_ port opts in
        let module AI_set =
          Set.Make(struct type t = addr_info let compare = compare end)
        in
        let ais = AI_set.elements
          (List.fold_left (fun s ai -> AI_set.add ai s) AI_set.empty ais) in
        (match ais with
          | [] ->
              Printf.ksprintf failwith
                "OBus_server.make: no address info for bind=%s port=%s%s"
                bind_ port
                (match OBus_address.arg "family" address with
                   | None -> "" | Some f -> " family=" ^ f)
          | ais ->
              let%lwt results =
                Lwt_list.map_p
                  (fun ai ->
                     try%lwt
                       ...
                       (match getsockname (Lwt_unix.unix_file_descr fd) with
                          ...
                          | ADDR_INET(host, port) ->
                              Lwt.return (`Success (fd, OBus_address.make ~name
                                ~args:[("host", string_of_inet_addr host);
              ...
              let successes =
                OBus_util.filter_map
                  (function `Success x -> Some x | `Failure _ -> None)
                  results
              in
              if successes = [] then
                match OBus_util.find_map
                        (function `Failure e -> Some e | _ -> None) results with
                  | Some exn -> Lwt.fail exn
                  | None     -> assert false
              else
                Lwt.return successes)
      end

    | "autolaunch" ->
        Lwt.fail (Failure
          "OBus_server.make: autolaunch cannot be used as a listening address")

    | name ->
        Lwt.fail (Failure ("OBus_server.make: unknown transport type: " ^ name))
...
let shutdown server =
  if server.srv_up then begin
    ...
    let%lwt () =
      if server.srv_nonce_file <> "" then
        try%lwt
          Lwt_unix.unlink server.srv_nonce_file
        with Unix_error(err, _, _) ->
          Lwt_log.error_f ~section "cannot unlink '%s': %s"
            server.srv_nonce_file (Unix.error_message err)
      else
        Lwt.return ()
    in
    server.srv_loops
...
let default_address =
  OBus_address.make ~name:"unix"
    ~args:[("tmpdir", Filename.get_temp_dir_name ())]

let make_lowlevel
    ?switch
    ?(capabilities=OBus_auth.capabilities)
    ?mechanisms
  ...
  match addresses with
    ...
    | addresses ->
                       ...
                       (fun (fd, address) ->
                          ...
                          with Unix_error(err, _, _) ->
                            Lwt_log.error_f ~section
                              "failed to close listening file descriptor: %s"
                              (Unix.error_message err))
                       fd_addr_list
                 | `Failure _ ->
                     Lwt.return ())
        ...
        in
        (match OBus_util.find_map
                 (function `Success _ -> None | `Failure e -> Some e)
                 result_by_address
         with
           | Some exn -> abort exn
           | None ->
               let%lwt nonce, nonce_file =
                 if List.exists
                      (fun addr -> OBus_address.name addr = "nonce-tcp")
                      addresses
                 then begin
                   ...
                   try%lwt
                     let%lwt () =
                       Lwt_io.with_file ~mode:Lwt_io.output file_name
                         (fun oc -> Lwt_io.write oc nonce)
                     in
                     Lwt.return (nonce, file_name)
                   with Unix.Unix_error(err, _, _) ->
                     abort (Failure(Printf.sprintf
                       "cannot create nonce file '%s': %s"
               ...
               let successes =
                 List.map
                   (function `Failure _ -> assert false | `Success x -> x)
                   result_by_address
               in
                 ...
                 List.map2
                   (fun fd_addr_list guid ->
                      List.map
                        (fun (fd, addr) ->
                           let args =
                             ("guid", OBus_uuid.to_string guid) :: OBus_address.args addr
                           in
                           let args =
                           ...
                           in
                           (fd, { addr with OBus_address.args }))
                        fd_addr_list)
                   successes guids
               in
               let listeners =
               ...
               server.srv_loops <-
                 Lwt.join (List.map (lst_loop server) listeners);
               let%lwt () =
                 Lwt_switch.add_hook_or_exec switch (fun () -> shutdown server)
               in
               Lwt.return server)
...
let make ?switch ?capabilities ?mechanisms ?addresses ?allow_anonymous callback =
  make_lowlevel ?switch ?capabilities ?mechanisms ?addresses ?allow_anonymous
    (fun server transport ->
       callback server (OBus_connection.of_transport ~up:false transport))
